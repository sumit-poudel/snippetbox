...
5.  Error wrapping and comparison
    - Wrap errors with fmt.Errorf("context: %w", err) to add context without losing the original error
    - The message string is readable across all layers for free — only the underlying error value needs unwrapping
    - Use errors.Is when comparing against a package-level var (sentinel — one known instance)
      -e.g. errors.Is(err, sql.ErrNoRows)
    - Use errors.As when comparing against a type (struct — instance created somewhere in the chain you don't have a reference to)
      -e.g. errors.As(err, &target) where target \*InvalidDecoderError
      -&target is needed so errors.As can write the found instance into your variable
    - Rule of thumb: var → Is, type → As
